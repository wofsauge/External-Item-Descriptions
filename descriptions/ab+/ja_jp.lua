...
EID.descriptions[languageCode].collectibles={
    ...
    {"34", "The Book of Belial", "↑ 攻撃力 +2#↑ 悪魔部屋／天使部屋の 出現率 +12.5%#↑ {{Collectible7}}殉教者の血 所有時、 攻撃力が追加で50%増"},
    ...
    {"120", "Odd Mushroom", "↑ 連射速度 +1.7#↑ 移動速度 +0.3#↓ 攻撃力　 -10%#↓ 攻撃力　 -0.4"},
    {"121", "Odd Mushroom", "↑ 最大体力 +1#↑ 攻撃力　 +0.3#↑ 射程　　 +0.25#↑ 涙の高さ +0.5#↓ 移動速度 -0.1"},
    ...
    {"135", "IV Bag", "いつでも売血#1ダメージ = 1～3コイン"},
    ...
    {"152", "Technology 2", "右目の涙が連続照射の 貫通レーザーに置き換わる#↓ 攻撃力 -35%"},
    ...
    {"161", "Ankh", "↑ 追加ライフ +1#死ぬと、一つ前の部屋に ？？？として復活する"},
    ...
    {"202", "Midas' Touch", "接触した敵に所持金と 等しいダメージを与え、 黄金化し、動きを止める#黄金化状態の敵を倒すと 1～4枚のコインを落とす"},
    ...
    {"212", "Guppy's Collar", "死ぬと、50%の確率で 1つ前の部屋に復活する"},
    {"213", "Lost Contact", "涙がシールドに包まれ、 敵の弾をブロックする#↓ 弾速 -0.16"},
    ...
    {"238", "Key Piece 1", "!!! メガサタンのドアを開く 鍵のかけら#↑ 天使部屋の出現率アップ"},
    {"239", "Key Piece 2", "!!! メガサタンのドアを開く 鍵のかけら#↑ 天使部屋の出現率アップ"},
    ...
    {"252", "Little Baggy", "ピルを2枚まで 保持可能になる#全てのカードをピルに 置き変える#取得時、ランダムな ピルを1個スポーン"},
    ...
    {"278", "Dark Bum", "1.5個の赤ハートを集める ごとに、黒／青ハート・ 青／敵クモのいずれかを スポーンする使い魔"},
    ...
    {"291", "Flush!", "全ての敵をうんちに変える#うんちタイプのボスを即死 させる"},
    ...
    {"297", "Pandora's Box", "{{Warning}} 使い切りアイテム#使用フロアで効果が変化：#B1:青ハート 2個#B2:爆弾と鍵 各2個#C1:ボスアイテム 1個#C2:B1+C1#D1:青ハート 4個#D2:コイン 20枚#W1:ボスアイテム 2個#W2:{{Collectible33}} 聖書#???/Void:なし#Sheol:悪魔部屋アイテム1個 　　　黒ハート 1個#DarkRoom:{{Collectible523}}お引越し用の箱 　　　　　 をアンロック#Cat:天使部屋アイテム 1個 　　白ハート 1個#Chest:コイン 1枚"},
    ...
    {"311", "Judas' Shadow", "↑ 追加ライフ +1#死ぬと、一つ前の部屋に ダークユダとして復活#復活後、実績上はユダ として扱われる"},
    ...
    {"372", "Charged Baby", "バトル中、30秒おきに、#{{Blank}} \7 電池のスポーン#{{Blank}} \7 チャージの追加#{{Blank}} \7 敵を1.5秒間石化#{{Blank}} のいずれかを行う使い魔"},
    ...
    {"384", "Lil Gurdy", "攻撃に連動してチャージを 溜め、解放されると高速で 体当たりを始める使い魔#毎秒90の接触ダメージ"},
    ...
    {"413", "Immaculate Conception", "ハートを15個拾うと、天使 タイプの使い魔が永続付与 され、同時に、青ハートが 1個スポーンする#使い魔は最大5体まで付与"},
    ...
    {"419", "Teleport 2.0", "未探索のランダムな 部屋にテレポートする#優先順位: {{Room}}>{{BossRoom}}>{{SuperSecretRoom}}>{{Shop}}>{{TreasureRoom}}>{{SacrificeRoom}}> {{DiceRoom}}>{{Library}}>{{CursedRoom}}>{{MiniBoss}}>{{ChallengeRoom}}>{{IsaacsRoom}}> {{ArcadeRoom}}>{{ChestRoom}}>{{SecretRoom}}>{{DevilRoom}}{{AngelRoom}}>I AM ERROR"},
    ...
    {"432", "Glitter Bombs", "爆弾 +5#爆発した爆弾が、ピック アップやトリンケットを 時々スポーンする"},
    ...
    {"450", "Eye of Greed", "20回に1回、涙と一緒に コインが発射され、敵を 黄金化し、動きを止める#発射時にコインが1枚 消費されるが、未所持 でもコインは発射される#黄金化状態の敵を倒すと 1～4枚のコインを落とす"},
    ...
    {"487", "Potato Peeler", "使用するたび：#↓ 最大体力 -1#↑ 攻撃力 +0.2#{{Collectible73}}肉の塊を付与"},
    ...
    {"507", "Sharp Straw", "部屋の全ての敵に ダメージを与える#ダメージ量は攻撃力+ 敵の最大体力の10%分#この効果でダメージを 与えた時、15%の確率で 半赤ハートをスポーン"},
    ...
    {"535", "Blanket", "青ハート +1#体力を1回復#ボス部屋に入った時、 一度だけダメージを 無効化するシールド を付与　　"},
...
EID.descriptions[languageCode].cards={
    ...
    {"34", "Ehwaz", "次のフロアにつながる トラップドアを生成#10%の確率でクロール スペースにつながる"},
    ...
    {"42", "Chaos Card", "移動方向にカードを投げ、 触れた敵を即死させる"},
...
EID.descriptions[languageCode].sacrifice={
    {"1", "", "50%の確率でコインを1枚 スポーンする"},
    {"2", "", "50%の確率でコインを1枚 スポーンする"},
...
EID.descriptions[languageCode].VoidNames = {"移動速度", "連射速度", "攻撃力", "射程", "弾速", "運"}
