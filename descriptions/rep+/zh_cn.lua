...
local collectibles = {
	...
	[141] = {"141", "盛装男孩", "{{Coin}} 生成7随机硬币#{{Trinket}} 生成一个硬币类饰品"}, -- Pageant Boy
	-- Change: added Fire rate information
	[186] = {"186", "血之权利", "{{Timer}} 使用后, 在当前房间中获得:↑ {{Tears}} 射速+0.48#对所有敌人造成40点伤害#{{Warning}} 对角色造成1心伤害#{{Heart}} 优先消耗红心"}, -- Blood Rights
	...
	[420] = {"420", "黑色粉末", "在地上走1个圈会产生五角星符号, 4秒内造成130点接触伤害#角色的炸弹变为大爆弹{{Collectible106}}#火堆在熄灭时会爆炸" }, -- Black Powder
	-- Change: Complete rewrite
	...
	[475] = {"475", "计划C", "对所有敌人造成9,999,999点伤害#{{Warning}} 在3秒后击杀角色#对母亲或精神错乱使用时延长至10到12秒"}, -- Plan C
...
local trinkets = {
	...
	[5] = {"5", "紫心奖章", "出现精英怪的几率翻倍#精英怪掉落更多战利品"}, -- Purple Heart
	-- Change: Added info about devil deals
	[7] = {"7", "念珠段", "{{AngelChance}} 天使房转换几率+100%#{{Collectible33}} 在{{Shop}}商店和{{Library}}图书馆出现圣经的几率更高"}, -- Rosary Bead
	-- Change: added info about dropping the item
	[16] = {"16", "妈妈的脚趾甲", "每20秒妈腿会践踏房间中的敌人#丢下/切换该饰品会在其掉落的位置召唤妈妈的腿"}, -- Mom's Toenail
	-- Change: added Super Secret Room info
	[23] = {"23", "寻人启事", "{{Player10}} 死亡时, 以游魂重生#{{SuperSecretRoom}} 33%几率揭示新一层的超级隐藏房"}, -- Missing Poster
	...
	[66] = {"66", "懒散虫", "↓ {{Shotspeed}} 弹速-0.5#↑ {{Damage}} 伤害+0.5"}, -- Lazy Worm
	-- Change: Complete rewrite
	[70] = {"70", "虱子", "在有敌人的房间中, 时不时生成1只蓝蜘蛛#清除一个房间后生成一只蓝蜘蛛"}, -- Louse
	-- Change: Added more loot information
	[76] = {"76", "筹码", "↑ 箱子有50%几率生成额外掉落物#↓ 箱子有50%几率只有1只红苍蝇#略微提升赌博时生成奖励的几率#如果打开的箱子中生成了道具且道具品质为0~2，将其重置为品质更高的道具#可以重置为该箱子道具池以外的道具"}, -- Poker Chip
	-- Change: Added additional effects
	[85] = {"85", "业报", "{{DonationMachine}} 降低捐款机卡住的概率#与任意捐献类实体交互时有33%几率:#{{Coin}} 获得1个硬币(40%)#{{HealingRed}} 治疗1红心(40%)#{{Luck}} 幸运+1(15%)#{{Beggar}} 生成1个乞丐(5%)"}, -- Karma
	-- Change: Added damage up information
	[89] = {"89", "儿童栓绳", "跟班离角色更近#跟班获得:↑ {{Damage}} 伤害修正x1.25"}, -- Child Leash
	-- Change: 33% chance, Spawns blue fly on new room
	[93] = {"93", "用过的尿布", "每房间15%几率使所有苍蝇敌人变得友好#进入新房间时，生成1个蓝苍蝇"}, -- Used Diaper
	-- Change: Changed "12-20 times" to "6-12 times"
	[97] = {"97", "扁桃体", "受伤6-12次后, 获得阻挡弹幕的跟班#最多+2个跟班"}, -- Tonsil
	-- Change: chance scales with luck
	[99] = {"99", "超级弹球", "10%几率发射弹性泪弹#{{Luck}} 幸运18: 100%几率"},
	-- Change: Added +2 Tears
	[103] = {"103", "等号！", "当角色的{{Coin}}硬币, {{Bomb}}炸弹和{{Key}}钥匙数量相等时:#将掉落物变成成对掉落物#↑ {{Tears}} 射速+2"}, -- Equality!
	...
	[172] = {"172", "诅咒硬币", "{{Coin}} 拾起硬币时, 将角色传送至随机房间#硬币的类型会影响传送的效果"}, -- Cursed Penny
}
...
local cards = {
	...
	[32] = {"32", "冰雹符文", "摧毁房间中所有石头及石头类敌人"}, -- Hagalaz
	-- Change: Complete rewrite
...
local horsepills = {
	...
	[32] = {"31", "？？？", "{{CurseMaze}} 本层施加混乱诅咒的效果#额外获得1个未拥有的诅咒"}, -- ???
	-- Change: affects whole floor
	[42] = {"41", "好困…", "{{Slow}} 当前层中使所有敌人减速"}, -- I'm Drowsy...
	-- Change: affects whole floor
	[43] = {"42", "好兴奋！！！", "当前层中使所有敌人加速"}, --I'm Excited!!
	-- Change: Forces the effect to be a golden trinket
	[44] = {"43", "咕噜！", "{{Trinket}} 消耗角色的饰品并永久获得其{{ColorGold}}金色{{CR}}效果"}, -- Gulp!
}
...
local carBattery = {
	...
	[47] = "落下2枚导弹", -- Doctor's Remote
	-- Added info regarding doubled attack
	[164] = "额外发射一团射程较短的蓝色火焰", -- The Candle
	-- Added info regarding doubled attack
	[289] = "额外发射一团射程较短的火焰", -- Red Candle
