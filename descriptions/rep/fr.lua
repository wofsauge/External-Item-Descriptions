...
local repCollectibles={
	...
	[34] = {"34", "Livre de Bélial", "{{DevilChance}} Chance de Pacte avec le Diable {{ColorLime}}+12.5%#↑ Dégâts{{ColorLime}} +2#{{Timer}} L'effet dure une salle"}, -- Book of Belial
	...
	[45] = {"45", "Joli Cœur", "Soigne un cœur rouge"}, -- Yum Heart
	...
	[75] = {"75", "Doctorat", "Soigne 2 cœurs rouges#Fait apparaître une pilule#Les pilules ne peuvent plus avoir d'effet négatif#Les banques du sang donnent une pièce de plus"},
	...
	[107] = {"107", "Ciseaux Cranteurs", "Permet à Isaac de voler#Le corps d'Isaac se sépare de sa tête, pourchasse les ennemis et inflige des dégâts de contact#{{DamageSmall}} 23.5 dégâts par seconde"},
	...
	[238] = {"238", "1er Fragment de Clé", "Combiner ce fragment de clé à l'autre permet d'ouvrir la grande porte dans le {{ColorTransform}}Coffre{{ColorWhite}} et la {{ColorTransform}}Chambre Noire #Augmente la fréquence d'apparition des cœurs éternels#{{AngelChanceSmall}} Augmente les chances d'Offrandes des Anges {{AngelRoom}}"},
	[239] = {"239", "2è Fragment de Clé", "Combiner ce fragment de clé à l'autre permet d'ouvrir la grande porte dans le {{ColorTransform}}Coffre{{ColorWhite}} et la {{ColorTransform}}Chambre Noire #Augmente la fréquence d'apparition des cœurs éternels#{{AngelChanceSmall}} Augmente les chances d'Offrandes des Anges {{AngelRoom}}"},
...
--	[297] = {"297", "Boîte de Pandore", "{{Warning}} USAGE UNIQUE#{{Blank}} Donne selon l'étage :#{{ColorTransform}}SS1{{ColorWhite}} : 2 cœurs d'âme#{{ColorTransform}}SS2{{ColorWhite}} : 2 clés, 2 bombes#{{ColorTransform}}G1{{ColorWhite}} : 1 Objet de boss#{{ColorTransform}}G2{{ColorWhite}} : SS1 + G1#{{ColorTransform}}P1{{ColorWhite}} : 4 cœurs d'âme#{{ColorTransform}}P2{{ColorWhite}} : 30 pièces#{{ColorTransform}}U1{{ColorWhite}} : 2 Objets de boss#{{ColorTransform}}U2{{ColorWhite}} : La {{ColorYellow}}Bible{{ColorWhite}} {{Collectible33}}#{{ColorTransform}}Shéol{{ColorWhite}} : 1 Objet du Diable, 1 cœur noir#{{ColorTransform}}Cathédrale{{ColorWhite}} : 1 Objet d'Ange + 1 cœur d'âme#{{ColorTransform}}Coffre{{ColorWhite}} : 1 Pièce#{{ColorTransform}}Chambre Noire{{ColorWhite}} : Débloque le{{ColorYellow}}Carton de Déménagement{{ColorWhite}} {{Collectible523}}#{{ColorTransform}}Maison{{ColorWhite}} : La {{ColorYellow}}Clé Rouge{{ColorWhite}} {{Collectible580}}"}, -- Pandora's box
	[297] = {"297", "Boîte de Pandore", "{{Warning}} USAGE UNIQUE#{{Blank}} Donne selon l'étage :#SS1 : {{SoulHeart}}{{SoulHeart}}#SS2 : {{Key}}{{Key}} + {{Bomb}}{{Bomb}}#Gr1 : Objet de boss#Gr2 : Objet de boss + {{SoulHeart}}{{SoulHeart}}#Pr1 : {{SoulHeart}}{{SoulHeart}}{{SoulHeart}}{{SoulHeart}}#Pr2 : 30{{Coin}}#Ut1 : 2 Objets de boss#Ut2 : La {{ColorYellow}}Bible {{Collectible33}}#??? / Néant : Rien#Shéol : Objet du Diable + {{BlackHeart}}#Cathédr. : Objet d'Ange + {{SoulHeart}}#Chambre Noire : Débloque le {{ColorYellow}}Carton de Déménagement {{Collectible523}}#Coffre : {{Coin}}#Maison : La {{ColorYellow}}Clé Rouge {{Collectible580}}"}, -- Pandora's box
	...
	[483] = {"483", "Mama Méga", "{{Warning}} USAGE UNIQUE#{{Blank}} Explose et ravage toutes les salles de l'étage#Si Isaac possède une bombe dorée, utiliser Mama Méga la consomme et permet à l'objet d'être utilisé une seconde fois"},
	...
	[635] = {"635", "Marionnette", "Invoque un familier déplaçable avec les boutons de tir#Utiliser l'objet inverse la position d'Isaac avec celle du familier#Se téléporter peut détruire ou infliger des dégâts à ce qui se trouve au point d'arrivée"}, --  Stitches
...
EID.descriptions[languageCode].CraftingWarningAvailableItems = "Il doit y avoir au moins 8 ressources pour afficher les recettes !"
...
local repBFFSSynergies = {
	...
	["5.100.702"] = "Double les dégâts des feux follets", -- Vengeful Spirit
...
local repCharacterInfo = {
	...
	[19] = {"Jacob", "Jacob et Ésaü sont contrôlés en même temps#Utiliser une bombe en pose une sous chaque personnage#Ésaü reste immobile si {{ButtonRT}} est maintenu#{{ButtonLT}} utilise l'activable de Jacob, {{ButtonRB}} utilise l'activable d'Ésaü, {{ButtonRT}} permet d'utiliser sa carte ou pilule#Face à un choix d'objets, chaque personnage peut en prendre un s'ils les touchent en même temps"},
	[20] = {"Ésaü", "Jacob et Ésaü sont contrôlés en même temps#Utiliser une bombe en pose une sous chaque personnage#Ésaü reste immobile si {{ButtonRT}} est maintenu#{{ButtonLT}} utilise l'activable de Jacob, {{ButtonRB}} utilise l'activable d'Ésaü, {{ButtonRT}} permet d'utiliser sa carte ou pilule#Face à un choix d'objets, chaque personnage peut en prendre un s'ils les touchent en même temps"},
	...
	[31] = {"L'Égaré Impur", "Les objets défensifs ou inutiles à l'Égaré ne peuvent pas apparaître#Les cartes ont 10% de chances d'être converties en Cartes Sacrées#Les objets de qualité {{Quality2}} ou inférieure ont 20% de chances d'être rejoués"},
	[32] = {"Lilith Impure", "Appuyer sur un bouton de tir projette un fœtus qui inflige {{ColorLime}}Dégâts x3#Maintenir un bouton de tir fait tirer le fœtus en direction de l'ennemi le plus proche"},
	[33] = {"Le Gardien Impur", "Tuer un ennemi fait apparaître une pièce temporaire#La plupart des objets coute 15 pièces#Les Pactes avec le Diable et les Offrandes des Anges coutent 15 ou 30 pièces#Les boutiques sont toujours ouvertes et proposent plus de choix"},
	[34] = {"Apollyon Impur", ""},
