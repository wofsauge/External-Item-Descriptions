...
local repCollectibles={
	...
	[75] = {"75", "Doctorat", "Soigne 2 cœurs rouges#Fait apparaître une pilule#Les pilules ne peuvent plus avoir d'effet négatif#Les machines de don du sang {{BloodDonationMachine}} et {{ColorYellow}}Perfusion{{ColorWhite}} {{Collectible135}} donnent davantage de pièces"},
...
--	[297] = {"297", "Boîte de Pandore", "{{Warning}} USAGE UNIQUE#{{Blank}} Donne selon l'étage :#{{ColorTransform}}SS1{{ColorWhite}} : 2 cœurs d'âme#{{ColorTransform}}SS2{{ColorWhite}} : 2 clés, 2 bombes#{{ColorTransform}}G1{{ColorWhite}} : 1 Objet de boss#{{ColorTransform}}G2{{ColorWhite}} : SS1 + G1#{{ColorTransform}}P1{{ColorWhite}} : 4 cœurs d'âme#{{ColorTransform}}P2{{ColorWhite}} : 30 pièces#{{ColorTransform}}U1{{ColorWhite}} : 2 Objets de boss#{{ColorTransform}}U2{{ColorWhite}} : La {{ColorYellow}}Bible{{ColorWhite}} {{Collectible33}}#{{ColorTransform}}Shéol{{ColorWhite}} : 1 Objet du Diable, 1 cœur noir#{{ColorTransform}}Cathédrale{{ColorWhite}} : 1 Objet d'Ange + 1 cœur d'âme#{{ColorTransform}}Coffre{{ColorWhite}} : 1 Pièce#{{ColorTransform}}Chambre Noire{{ColorWhite}} : Débloque {{ColorYellow}}Carton de Déménagement{{ColorWhite}} {{Collectible523}}#{{ColorTransform}}Maison{{ColorWhite}} : La {{ColorYellow}}Clé Rouge{{ColorWhite}} {{Collectible580}}"}, -- Pandora's box
	...
	[350] = {"350", "Choc Toxique", "Entrer dans une salle empoisonne tous les ennemis#Les ennemis répandent une flaque toxique à leur mort#Rend Isaac invulnérable aux nuages de poison"}, -- Toxic Shock
	...
	[523] = {"523", "Carton de Déménagement", "Utiliser l'objet absorbe jusqu'à 10 ressources, babioles et objets dans la salle#Le prochaine utilisation de l'objet posera les objets absorbés sur le sol"}, -- Moving Box
	[524] = {"524", "Technologie Zéro", "Les larmes d'Isaac sont reliées par des arcs électriques"}, -- Technology Zero
	...
	[645] = {"645", "P'tit Tératome", "Orbite autour d'Isaac et bloque les projectiles#Après avoir reçu 3 coups, se sépare en parties plus petites puis en araignées bleues quand elles meurent#Réapparaît 5 secondes après sa disparition complète"}, --  Tinytoma
	...
	[690] = {"690", "Haricot Confit", "Les ennemis qui entrent en contact avec Isaac rebondissent sur son ventre#50% de chances de nullifier un dégât subi au contact#50% de chances de renvoyer les projectiles ennemis"}, --  Belly Jelly
	...
	[724] = {"724", "Hypercoagulation", "Subir des dégâts fait apparaître un cœur ou demi-cœur rouge#Ces cœurs disparaissent après 1.5 secondes s'ils ne sont pas ramassés"}, --  Hypercoagulation
	[725] = {"725", "Estomac Irrité", "Infliger suffisamment de dégâts fait clignoter Isaac en rouge#Relâcher les boutons de tir quand Isaac clignote utilise un caca spécial"}, --  IBS
...
local repCards={
	...
	[39] = {"39", "Algiz", "Isaac devient invulnlérable pendant 20 secondes"},
	...
	[66] = {"66", "X - La Roue de Fortune ?", "Déclenche l'effet d'une salle de dé aléatoire"},
