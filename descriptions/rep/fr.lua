...
local repCollectibles={
	...
	[22] = {"22", "Déjeuner", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- Lunch
	[23] = {"23", "Dîner", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- Dinner
	[24] = {"24", "Dessert", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- Dessert
	[25] = {"25", "Petit Déjeuner", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- Breakfast
	[26] = {"26", "Viande Avariée", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- Rotten Meat
	...
	[33] = {"33", "Bible", "Permet à Isaac de voler#{{Timer}} L'effet dure une salle#Tue instantanément {{ColorCyan}}Maman{{ColorWhite}}, le {{ColorCyan}}Cœur de Maman{{ColorWhite}} et {{ColorCyan}}Il est Vivant{{ColorWhite}}#{{Warning}} Tue Isaac si utilisée contre {{ColorCyan}}Satan"}, -- The Bible
	...
	[45] = {"45", "Joli Cœur", "{{HealingRed}} Soigne un cœur rouge"}, -- Yum Heart
	...
	[138] = {"138", "Stigmates", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#↑ Dégâts {{ColorLime}}+0.3"}, -- Stigmata
	...
	[176] = {"176", "Cellules Souches", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#↑ Vitesse des tirs {{ColorLime}}+0.16"}, -- Stem Cells
	...
	[184] = {"184", "Saint Graal", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#Permet à Isaac de voler"}, -- Holy Grail
	...
	[193] = {"193", "DE LA VIANDE !", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#↑ Dégâts {{ColorLime}}+0.3"}, -- MEAT!
	...
	[218] = {"218", "Placenta", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#{{HealingRed}} Soigne parfois un demi-cœur rouge quand le minuteur finit par :00"}, -- Placenta
	...
	[226] = {"226", "Lotus Noir", "{{Heart}} +1 réceptacle de cœur#{{SoulHeart}} +1 cœur d'âme#{{BlackHeart}} +1 cœur noir#{{HealingRed}} Soigne un cœur rouge"}, -- Black Lotus
	...
	[253] = {"253", "Croûte Magique", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#↑ Chance {{ColorLime}}+1"}, -- Magic Scab
...
--	[297] = {"297", "Boîte de Pandore", "{{Warning}} USAGE UNIQUE {{Warning}}#{{Blank}} Donne selon l'étage :#{{ColorTransform}}SS1{{ColorWhite}} : 2 cœurs d'âme#{{ColorTransform}}SS2{{ColorWhite}} : 2 clés, 2 bombes#{{ColorTransform}}G1{{ColorWhite}} : 1 Objet de boss#{{ColorTransform}}G2{{ColorWhite}} : SS1 + G1#{{ColorTransform}}P1{{ColorWhite}} : 4 cœurs d'âme#{{ColorTransform}}P2{{ColorWhite}} : 30 pièces#{{ColorTransform}}U1{{ColorWhite}} : 2 Objets de boss#{{ColorTransform}}U2{{ColorWhite}} : La {{ColorYellow}}Bible{{ColorWhite}} {{Collectible33}}#{{ColorTransform}}Shéol{{ColorWhite}} : 1 Objet du Diable, 1 cœur noir#{{ColorTransform}}Cathédrale{{ColorWhite}} : 1 Objet d'Ange + 1 cœur d'âme#{{ColorTransform}}Coffre{{ColorWhite}} : 1 Pièce#{{ColorTransform}}Chambre Noire{{ColorWhite}} : Débloque le{{ColorYellow}}Carton de Déménagement{{ColorWhite}} {{Collectible523}}#{{ColorTransform}}Maison{{ColorWhite}} : La {{ColorYellow}}Clé Rouge{{ColorWhite}} {{Collectible580}}"}, -- Pandora's box
	...
	[314] = {"314", "Seau de Fromage", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#↓ Vitesse {{ColorError}}-0.4#Marcher sur les obstacles les détruit"}, -- Thunder Thighs
	...
	[342] = {"342", "Lactarius Indigo", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#↑ Débit {{ColorLime}}+0.7#↓ Vitesse des tirs {{ColorError}}-16%"}, -- Blue Cap
	...
	[346] = {"346", "Casse-Croûte", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- A Snack
	...
	[354] = {"354", "Boîte de Céréales", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge#Fait apparaître une babiole"}, -- Crack Jacks
	...
	[456] = {"456", "Encas de Minuit", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, -- Midnight Snack
	...
	[664] = {"664", "Goinfrerie", "{{Heart}} +1 réceptacle de cœur#Les objets sur piédestal alternent avec un objet de nourriture#Choisir la nourriture donne {{ColorLime}}dégâts +3.6{{ColorWhite}},{{ColorError}} vitesse -0.03{{CR}} et augmente deux stats#Ces bonus de dégâts s'estompent avec le temps"}, --  Binge Eater
	...
	[707] = {"707", "Souper", "{{Heart}} +1 réceptacle de cœur#{{HealingRed}} Soigne un cœur rouge"}, --  Supper
...
EID.descriptions[languageCode].bookOfVirtuesWisps = {
	[33] = "Les feux {{InnerWisp}} tirent des larmes autoguidées", -- The Bible
	[34] = "Ces feux {{MiddleWisp}}  follets infligent le double de dégâts", -- The Book of Belial
	[35] = "Les feux {{InnerWisp}}  follets tirent parfois une larme terrifiante et déclenchent l'effet du Necronomicon une fois détruits", -- The Necronomicon
	[36] = "Les feux {{OuterWisp}}  follets tirent parfois une larme E-coli", -- The Poop
	[37] = "Les feux {{MiddleWisp}}  follets font apparaître de petites bombes autoguidées une fois détruits", -- Mr. Boom
	[38] = "Les feux {{OuterWisp}}  follets tirent des larmes rapides mais peu précises", -- Tammy's Head
	[39] = "Les feux {{MiddleWisp}}  follets tirent parfois une larme pétrifiante", -- Mom's Bra
	[40] = "Les feux {{MiddleWisp}}  follets tirent des larmes explosives et explosent une fois détruits", -- Kamikaze!
	[41] = "Les feux {{MiddleWisp}} follets tirent parfois une larme terrifiante", -- Mom's Pad
	[42] = "Les feux  {{MiddleWisp}} follets tirent parfois une larme Ipéca", -- Bob's Rotten Head
	[44] = "Les feux  {{MiddleWisp}} follets ont 10% de chances de téléporter un ennemi touché", -- Teleport!
	[45] = "Les ennemis tués par les feux {{MiddleWisp}} ont 20% de chances de lâcher un cœur", -- Yum Heart
	[47] = "Les feux  {{MiddleWisp}} follets infligent la moitié de dégâts mais lancent des bombes en direction du curseur", -- Doctor's Remote
	[49] = "Les feux  {{OuterWisp}} follets ne tirent pas et crachent un laser quand Isaac utilise l'objet", -- Shoop da Whoop!
	[56] = "Les feux  {{MiddleWisp}} follets tirent des larmes rapides à courte portée et laissent une traînée corrosive", -- Lemon Mishap
	[58] = "Les feux  {{MiddleWisp}} follets sont invulnérables aux projectiles", -- Book of Shadows
	[65] = "Les feux  {{MiddleWisp}} follets font apparaître une bombe amorcée une fois détruits", -- Anarchist Cookbook
	[66] = "Les feux  {{MiddleWisp}} follets ralentissent les ennemis pendant 3 secondes une fois détruits", -- The Hourglass
	[77] = "Les feux  {{MiddleWisp}} follets infligent plus de dégâts de contact", -- My Little Unicorn
	[78] = "Les feux  {{InnerWisp}} follets ne peuvent pas tirer et invoquent un P'tit Cavalier temporaire une fois détruits", -- Book of Revelations
	[83] = "Les feux  {{InnerWisp}} follets tirent des larmes répulsives", -- The Nail
	[84] = "Feux  {{MiddleWisp}} follets normaux", -- We Need to Go Deeper!
	[85] = "Les feux  {{InnerWisp}} follets lâchent une carte une fois détruits", -- Deck of Cards
	[86] = "Les feux  {{MiddleWisp}} follets tirent des dents", -- Monstro's Tooth
	[93] = "Les feux  {{InnerWisp}} follets tirent des larmes anti-gravité et parfois une larme terrifiante", -- The Gamekid
	...
	[102] = "Les feux {{InnerWisp}} infligent un effet différent aux ennemis en fonction de leur couleur", -- Mom's Bottle of Pills
	[105] = "Invoque un feu {{MiddleWisp}} pour chaque objet rejoué", -- The D6
	[107] = "Les feux {{InnerWisp}} tirent des larmes transperçantes", -- The Pinking Shears
	[111] = "Les feux {{OuterWisp}} pètent une fois détruits", -- The Bean
	[123] = "Les feux {InnerWisp}} tirent des larmes aléatoires", -- Monster Manual
	[124] = "Invoque le feu follet de l'objet imité", -- Dead Sea Scrolls
	[126] = "Les feux {{MiddleWisp}} sont très résistants", -- Razor Blade
	[127] = "Invoque un feu {{MiddleWisp}} invulnérable pour l'étage", -- Forget Me Now
	[130] = "Les feux {{MiddleWisp}} ne peuvent pas tirer mais foncent dans la direction des tirs d'Isaac", -- A Pony
	[133] = "3 feux {{MiddleWisp}} très résistants", -- Guppy's Paw
	[135] = "Feux {{OuterWisp}} normaux", -- IV Bag
	[136] = "Les feux {{MiddleWisp}} tirent parfois des larmes tomates#Les ennemis touchés sont pris pour cible par les autres ennemis", -- Best Friend
	[137] = "Invoque 1 feu follet {{InnerWisp}} par bombe détonée#Les larmes des feux peuvent être explosées à distance", -- Remote Detonator
	[145] = "À la place de mouches, invoque des feux {{OuterWisp}} qui se transforment en mouches bleues une fois détruits", -- Guppy's Head
	[146] = "Les feux {{InnerWisp}} tirent des larmes autoguidées", -- Prayer Card
	[147] = "Briser des obstacles peut invoquer 1 feu follet parmi 5 avec des effets différents", -- Notched Axe
	[158] = "Les feux {{MiddleWisp}} tirent sur les ennemis à proximité", -- Crystal Ball
	[160] = "Frappe d'un rayon de lumière les ennemis qui entrent en contact avec les feux {{MiddleWisp}}", -- Crack the Sky
	[164] = "Les feux {{MiddleWisp}} durent une salle", -- The Candle
	[166] = "Détruit toutes les ressources et peut invoquer un feu {{MiddleWisp}} pour chacune", -- D20
	[171] = "Les feux {{MiddleWisp}} tirent parfois une larme ralentissante", -- Spider Butt
	[175] = "Les feux {{MiddleWisp}} ouvrent toutes les serrures qu'ils touchent", -- Dad's Key
	[177] = "Invoque un feu follet différent en fonction de la récompense gagnée", -- Portable Slot
	[181] = "Les feux {{MiddleWisp}} tirent parfois une larme sacrée", -- White Pony
	[186] = "Les feux {{MiddleWisp}} sont très résistants", -- Blood Rights
	[192] = "Les feux {{MiddleWisp}} tirent des larmes autoguidées", -- Telepathy for Dummies
	[263] = "Les ennemis tués par les feux ont 15% de chances de lâcher une rune#Les feux {{MiddleWisp}} lâchent une rune une fois détruits", -- Clear Rune
	[282] = "Invoque jusqu'à 6 feux immobiles", -- How to Jump
	[283] = "Rejoue tous les feux et en ajoute 1#Ne rejoue pas le {{ColorYellow}}Livre des Vertus", -- D100
	[284] = "Détruit tous les feux et en invoque un par paire d'objets rejoués#Ne rejoue pas le {{ColorYellow}}Livre des Vertus", -- D4
	[285] = "Les feux {{MiddleWisp}} rejouent parfois un ennemi touché", -- D10
	[286] = "Les ennemis tués par ces feux lâchent parfois une carte#Les feux {{MiddleWisp}} lâchent une carte une fois détruits", -- Blank Card
	[287] = "Les feux {{MiddleWisp}} ouvrent les portes, révèlent une salle inexplorée sur la carte et brisent les rochers marqués de la salle une fois détruits", -- Book of Secrets
	[288] = "Au lieu d'araignées, invoque des feux {{OuterWisp}} qui se transforment en araignées bleues une fois détruits", -- Box of Spiders
	[289] = "Les feux {{MiddleWisp}} tirent des flammes rouges et disparaissent à la salle suivante", -- Red Candle
	[290] = "Crée un feu follet pour chaque cœur dans le bocal#Les feux {{MiddleWisp}} lâchent parfois un cœur une fois détruits", -- The Jar
	[291] = "Invoque un feu {{MiddleWisp}} pour chaque ennemi transformé", -- Flush!
	...
	[293] = "Les feux {{MiddleWisp}} tirent des lasers en croix une fois détruits", -- Head of Krampus
	[294] = "Les feux {{InnerWisp}} ne tirent pas, pètent une fois détruits et disparaissent à la salle suivante", -- Butter Bean
	[295] = "Les feux {{InnerWisp}} lâchent parfois une pièce une fois détruits", -- Magic Fingers
	[296] = "feux {{MiddleWisp}} normaux", -- Converter
	[297] = "feux {{MiddleWisp}} normaux", -- Pandora's Box
	[298] = "Les feux {{MiddleWisp}} ne peuvent pas tirer, sont immunisés aux dégâts de contact mais n'infligent pas de dégâts de contact tant que la Corne Brisée n'est pas active", -- Unicorn Stump
	[323] = "Invoque 6 feux {{MiddleWisp}} qui disparaissent après 3 secondes", -- Isaac's Tears
	[324] = "Les feux {{MiddleWisp}} tirent des larmes aléatoires", -- Undefined
	[325] = "Les feux {{MiddleWisp}} tirent des gerbes de larmes", -- Scissors
	[326] = "Un blocage parfait invoque 4 feux {{InnerWisp}}", -- Breath of Life
	[338] = "Les feux {{MiddleWisp}} tirent des larmes boomerang et durent une salle", -- The Boomerang
	[347] = "Duplique tous les feux et invoque un feu follet qui tire deux larmes", -- Diplopia
	[348] = "Les feux {{MiddleWisp}} lâchent une pilule une fois détruits", -- Placebo
	[349] = "Les feux {{MiddleWisp}} ont 50% de chances de lâcher une pièce une fois détruits", -- Wooden Nickel
	[351] = "Les feux {{MiddleWisp}} de cet objet génèrent une vague de rochers à chaque utilisation, pètent, pétrifient et empoisonnent les ennemis à proximité une fois détruits", -- Mega Bean
	[352] = "Les feux {{MiddleWisp}} de cet objet sont détruits si l'un est détruit", -- Glass Cannon
	[357] = "Invoque un feu {{MiddleWisp}} et duplique tous les autres pour une salle", -- Box of Friends
	[382] = "Les monstres alliés se transforment en feux qui correspondent à leurs capacités", -- Friendly Ball
	[383] = "Les feux {{MiddleWisp}} éclatent en 6 larmes une fois détruits et leurs larmes peuvent être divisées", -- Tear Detonator
	[386] = "Les larmes des feux {{MiddleWisp}} rejouent parfois un obstacle", -- D12
	[396] = "Un feu follet orbite autour de chaque trou et ne peut pas tirer", -- Ventricle Razor
	[406] = "Les feux {{MiddleWisp}} ont des dégâts et un débit aléatoires", -- D8
	[419] = "Les feux {{MiddleWisp}} téléportent parfois un ennemi touché", -- Teleport 2.0
	[421] = "Les feux {{MiddleWisp}} pètent et envoûtent les ennemis une fois détruits", -- Kidney Bean
	[422] = "Les feux {{MiddleWisp}} pétrifient parfois un ennemi touché", -- Glowing Hour Glass
	[427] = "Les feux rebondissent contre les murs et explosent s'ils sont touchés par les larmes d'Isaac", -- Mine Crafter
	[434] = "Invoque un feu {{MiddleWisp}} au lieu d'1 à 5 mouches bleues", -- Jar of Flies
	[437] = "Les feux {{MiddleWisp}} déclenchent l'effet du D7 au premier dégât subi", -- D7
	[439] = "feux {{MiddleWisp}} normaux", -- Mom's Box
	[441] = "Les feux {{MiddleWisp}} crachent des lasers de sang et tirent en continu quand Isaac utilise l'objet", -- Mega Blast
	[475] = "Invoque 8 feux {{MiddleWisp}} qui infligent de gros dégâts", -- Plan C
	[476] = "Invoque deux feux {{MiddleWisp}}, trois si aucune ressource n'a été dupliquée", -- D1
	[477] = "Invoque les feux de tous les objets absorbés", -- Void
	[478] = "Les feux {{MiddleWisp}} immobilisent les ennemis et projectiles pour 3 secondes une fois détruits", -- Pause
	[479] = "Les feux {{MiddleWisp}} font parfois apparaître une babiole une fois détruits", -- Smelter
	[480] = "Les PV du feu {{MiddleWisp}} invoqué dépendent du nombre de ressources converties", -- Compost
	[481] = "Les feux {{MiddleWisp}} peuvent transformer un ennemi touché en feu follet", -- Dataminer
	[482] = "Invoque un feu follet aléatoire", -- Clicker
	[483] = "Invoque 8 feux {{MiddleWisp}} qui lâchent des bombes troll dorées une fois détruits", -- Mama Mega!
	[484] = "Les feux {{MiddleWisp}} ne peuvent pas tirer, envoient une vague de rochers une fois détruits et disparaissent à la salle suivante", -- Wait What?
	[485] = "Tous Les feux {{MiddleWisp}} de cet objet sont détruits ou dupliqués quand l'un subit un dégât", -- Crooked Penny
	[486] = "Les feux {{MiddleWisp}} ne tirent pas et nullifient parfois un dégât infligé à Isaac", -- Dull Razor
	[487] = "Invoque des feux permanents qui pourchassent les ennemis pour leur tirer dessus", -- Potato Peeler
	...
	[504] = "Les feux {{MiddleWisp}} tirent sur les ennemis à proximité", -- Brown Nugget
	[507] = "Les ennemis tués peuvent se transformer en feux {{OuterWisp}}", -- Sharp Straw
	...
	[512] = "Les feux {{MiddleWisp}} tirent des larmes magnétisées", -- Black Hole
	[515] = "Invoque un feu follet aléatoire qui se régénère à chaque nouvelle salle", -- Mystery Gift
	[516] = "Les feux {{MiddleWisp}} tirent comme l'Arroseur Automatique", -- Sprinkler
	[521] = "Les feux {{MiddleWisp}} tirent 3 larmes et meurent si Isaac dépense de l'argent", -- Coupon
	[522] = "Transforme les projectiles interceptés en feux {{OuterWisp}}", -- Telekinesis
	[523] = "Les feux {{MiddleWisp}} ne sont invoqués qu'en déballant et lâchent une ressource une fois détruits", -- Moving Box
	[527] = "Les feux {{MiddleWisp}} ne peuvent pas tirer et vont ouvrir des serrures une fois la salle terminée", -- Mr. ME!
	[536] = "Convertit tous les feux en demi-cœurs et invoque deux feux {{MiddleWisp}}", -- Sacrificial Altar
	[545] = "Les feux {{InnerWisp}} invoquent un Osselet amical une fois détruits", -- Book of the Dead
	[550] = "Un ennemi touché par ces feux {{MiddleWisp}} peut se faire écraser par {{ColorCyan}}Maman{{ColorWhite}}", -- Broken Shovel
	[552] = "Un ennemi touché par ces feux {{MiddleWisp}} peut se faire écraser par {{ColorCyan}}Maman{{ColorWhite}}#Invoque aussi un feu follet plus résistant qui ne peut pas tirer", -- Mom's Shovel
	[555] = "Les feux {{MiddleWisp}} ont 15% de chances de transformer un ennemi touché en or", -- Golden Razor
	[556] = "Les feux {{MiddleWisp}} crachent de courts lasers de sang", -- Sulfur
	[557] = "↑ Chance {{ColorLime}}+0.2{{ColorWhite}} par feu {{MiddleWisp}} de cet objet", -- Fortune Cookie
	[577] = "feux {{MiddleWisp}} normaux", -- Damocles
	[578] = "Les feux {{MiddleWisp}} créent une trainée corrosive", -- Free Lemonade
	[580] = "Les feux {{MiddleWisp}} ouvrent parfois une porte rouge en entrant dans une salle", -- Red Key
	[582] = "Impossible d'invoquer plus d'un feu {{MiddleWisp}} de cet objet à la fois", -- Wavy Cap
	[584] = "Aucun effet", -- Book of Virtues
	[585] = "Invoque 8 feux {{MiddleWisp}}", -- Alabaster Box
	[604] = "Invoque 3 feux où atterrit l'obstacle#Les feux {{Wisp}} tirent parfois une larme étourdissante", -- Mom's Bracelet
	[605] = "feux {{MiddleWisp}} normaux", -- The Scooper
	[609] = "Utiliser l'objet a 50% de chances d'invoquer un feu {{MiddleWisp}} invulnérable ou de détruire tous ses feux", -- Eternal D6
	[611] = "Les dégâts et PV des feux {{MiddleWisp}} dépendent de la charge de l'objet", -- Larynx
	[622] = "Invoque 3 feux {{MiddleWisp}} et détruit tous les autres", -- Genesis
	[623] = "feux {{MiddleWisp}} normaux", -- Sharp Key
	[631] = "Invoque un feu {{MiddleWisp}} puis découpe les autres en deux et divise leurs PV et leurs dégâts par 2", -- Meat Cleaver
	[635] = "Invoque jusqu'à 6 feux immobiles au point de téléportation", -- Stitches
	[636] = "N'invoque pas de feu follet", -- R Key
	[638] = "Les feux {{MiddleWisp}} effacent les ennemis au contact", -- Eraser
	[639] = "Les feux {{MiddleWisp}} ne tirent pas et invoquent une mouche bleue à la fin d'une salle", -- Yuck Heart
	[640] = "Les feux {{MiddleWisp}} peuvent tirer des flammes bleues", -- Urn of Souls
	[642] = "Les feux {{MiddleWisp}} tirent des larmes empoisonnées", -- Magic Skin
	[650] = "Quand Prunelle rebondit, Les feux {{MiddleWisp}} de cet objet font de même", -- Plum Flute
	...
	[685] = "Double les feux  invoqués et en ajoute 1", -- Jar of Wisps
	[687] = "Invoque un feu follet aléatoire", -- Friend Finder
	[703] = "Feux {{MiddleWisp}} normaux", -- Esau Jr.
	[704] = "Les feux {{MiddleWisp}} sont très résistants et ne peuvent pas tirer", -- Berserk
	[705] = "Tuer un ennemi invoque un feu {{MiddleWisp}} qui disparaît à la salle suivante", -- Dark Arts
	[706] = "Les feux {{MiddleWisp}} sont très résistants et ne peuvent pas tirer", -- Abyss
	[709] = "3 feux sont invoqués à l'atterrissage et disparaissent à la salle suivante", -- Suplex
	[710] = "Fabriquer invoque un feu follet aléatoire", -- Bag of Crafting
	[711] = "Feux {{MiddleWisp}} normaux", -- Flip
	[712] = "Les feux objets peuvent tirer des larmes", -- Lemegeton
	[713] = "N'invoque pas de feu follet", -- Sumptorium
	[719] = "Un ennemi tué par ces feux {{MiddleWisp}} lâche parfois une pièce", -- Keeper's Box
	...
	[723] = "Rejoue tous les feux {{MiddleWisp}} en feux identiques", -- Spindown Dice
	[728] = "Tous les feux {{MiddleWisp}} orbitent autour de Gello", -- Gello
...
local repTrinkets={
	...
	[53] = {"53", "Tique", "{{HealingRed}} Entrer dans une salle de boss soigne un cœur rouge#Retire 15% des PV des boss#{{Warning}} Ne peut être retirée qu'avec l'{{ColorYellow}}Allumette{{ColorWhite}} {{Trinket41}}"},
