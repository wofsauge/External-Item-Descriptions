...
local repCollectibles={
	...
	[245] = {"245", "20/20", "↓ Dégâts {{ColorError}}-20%#Double tir#Annule la réduction de débit de {{ColorYellow}}Polyphème{{ColorWhite}} {{Collectible169}}, du {{ColorYellow}}Troisième Œil{{ColorWhite}} {{Collectible2}} et d'{{ColorYellow}}Araignée Mutante{{ColorWhite}} {{Collectible153}}"}, -- 20/20
...
--	[297] = {"297", "Boîte de Pandore", "{{Warning}} USAGE UNIQUE#{{Blank}} Donne selon l'étage :#{{ColorTransform}}SS1{{ColorWhite}} : 2 cœurs d'âme#{{ColorTransform}}SS2{{ColorWhite}} : 2 clés, 2 bombes#{{ColorTransform}}G1{{ColorWhite}} : 1 Objet de boss#{{ColorTransform}}G2{{ColorWhite}} : SS1 + G1#{{ColorTransform}}P1{{ColorWhite}} : 4 cœurs d'âme#{{ColorTransform}}P2{{ColorWhite}} : 30 pièces#{{ColorTransform}}U1{{ColorWhite}} : 2 Objets de boss#{{ColorTransform}}U2{{ColorWhite}} : La {{ColorYellow}}Bible{{ColorWhite}} {{Collectible33}}#{{ColorTransform}}Shéol{{ColorWhite}} : 1 Objet du Diable, 1 cœur noir#{{ColorTransform}}Cathédrale{{ColorWhite}} : 1 Objet d'Ange + 1 cœur d'âme#{{ColorTransform}}Coffre{{ColorWhite}} : 1 Pièce#{{ColorTransform}}Chambre Noire{{ColorWhite}} : Débloque {{ColorYellow}}Carton de Déménagement{{ColorWhite}} {{Collectible523}}#{{ColorTransform}}Maison{{ColorWhite}} : La {{ColorYellow}}Clé Rouge{{ColorWhite}} {{Collectible580}}"}, -- Pandora's box
	...
	[588] = {"588", "Sol", "Révèle l'emplacement de la salle de boss {{BossRoom}}#{{Blank}}#{{Blank}} Après avoir battu un boss:#↑ Dégâts {{ColorLime}}+3.0#↑ Chance {{ColorLime}}+1.0#Déclenche {{ColorYellow}}XIX - Le Soleil {{Card20}} #Recharge entièrment l'objet activable#Retire tous les fléaux"}, --  Sol
	...
	[652] = {"652", "Bébé Congelé", "Glisse quand il est poussé par Isaac#Ralentit les ennemis et leur inflige des dégâts de contact#Gèle les ennemis qu'il tue#Entrer en contact avec un ennemi gelé le fait glisser et exploser en 6 fragments de glace"}, --  Cube Baby
	...
	[725] = {"725", "Estomac Irrité", "Infliger suffisamment de dégâts fait clignoter Isaac en rouge#{{Blank}}#{{Blank}} Relâcher les boutons de tir quand Isaac clignote utilise un caca spécial"}, --  IBS
